...
class RegionSegmentation(object):
    ...
    def export_region_image(self):
        output_folder = os.path.join(self.settings.output_folder, 'region_export')
        ...
        filename = os.path.join(output_folder, 'export_segmented_regions_%s.png' % self.settings.dataset)
        skimage.io.imsave(filename, region_image)
        return

    def get_Bcell_image(self):
        si = SequenceImporter(['CD19'])
        img, channel_names = si(self.settings.input_folder)
        return img[:,:,0], channel_names

    def get_Tcell_image(self):
    ...
    def asf(self, img):
        filter_sizes = self.settings.prefilter_asf_sizes
        temp2 = img.copy()
        for i in range(len(filter_sizes)):
            temp1 = opening(temp2, disk(filter_sizes[i]))
            temp2 = closing(temp1, disk(filter_sizes[i]))
        return temp2

    def prefilter(self, img, method = 1):
        image_rescaled = rescale(img, 0.25)
        #pref_img = img.copy()
        if method == 1:
            pref1 = gaussian(image_rescaled, sigma = self.settings.prefilter_gaussian_bandwidth)
            pref_img = rescale(pref1, 4.0)

            if self.settings.debug:
                filename ='prefiltered_gaussian_%.2f.png' % self.settings.prefilter_gaussian_bandwidth
                self.save_debug_img(pref_img, filename, True)

        if method == 2:
            pref1 = area_close(image_rescaled.astype(np.uint8), self.settings.prefilter_area_closing)
            pref2 = gaussian(pref1, sigma = self.settings.prefilter_gaussian_bandwidth)
            pref_img = rescale(pref2, 4.0)

            if self.settings.debug:
                filename ='prefiltered_area_closing_gaussian_%.2f.png' % self.settings.prefilter_gaussian_bandwidth
                self.save_debug_img(pref_img, filename, True)

        if method == 3:
            
            pref1 = median(image_rescaled.astype(np.uint8), selem=np.ones((self.settings.prefilter_median_size,
                                                  self.settings.prefilter_median_size)))
            pref_img = rescale(pref1, 4.0)

            if self.settings.debug:
                filename ='prefiltered_median_%i.png' % self.settings.prefilter_median_size
                self.save_debug_img(pref_img, filename, True)
        ...
        if method == 4:
            ...
            if self.settings.debug:
                filename ='prefiltered_median_%i_gaussian_%.1f.png' % (self.settings.prefilter_median_size, 
                                                                       self.settings.prefilter_gaussian_bandwidth)
                self.save_debug_img(pref_img, filename, True)

        if method == 5:
            pref1 = gaussian(image_rescaled, sigma = self.settings.prefilter_gaussian_bandwidth)
            pref2 = self.asf(pref1)
            pref_img = rescale(pref2, 4.0)

            if self.settings.debug:
                filename ='prefiltered_asf_gaussian_%.2f.png' % self.settings.prefilter_gaussian_bandwidth
                self.save_debug_img(pref_img, filename, True)

        return pref_img

    def make_composite_BC(self, b_factor = 1.0, t_factor = 1.0):
        B_cell, names = self.get_Bcell_image()
        T_cell, names = self.get_Tcell_image()
        rgb_image = np.zeros((B_cell.shape[0], B_cell.shape[1], 3), dtype=np.float64)

        blue = b_factor * 255.0 / np.percentile(B_cell, 99.5) * B_cell
        blue[blue > 255.0] = 255.0
        blue[blue < 0.0] = 0.0

        green = t_factor * 255.0 / np.percentile(T_cell, 99.5) * T_cell
        ...
        #rgb_image[:,:,2] = b_factor * 255.0 / np.percentile(B_cell, 0.98) * B_cell 
        #rgb_image[:,:,1] = t_factor * 255.0 / np.percentile(T_cell, 0.98) * T_cell 
        rgb_image = rgb_image.astype(np.uint8)
        #pdb.set_trace()
        if self.settings.debug:
            self.save_debug_img(rgb_image, 'B_C_composite.png', False)

        return rgb_image
    
    def T_cell_segmentation(self, img=None):
        if img is None:
            img = self.get_Tcell_image()
        pref = self.prefilter(img, method=1)
        seg1 = pref > self.settings.thresh_alpha * pref.mean()
        seg2 = remove_small_holes(seg1, 10000)
        seg3 = remove_small_objects(seg2, 40000)
        seg4 = opening(seg3, disk(10))
        segmentation = closing(seg4, disk(8))
        #segmentation = self.asf(seg3)
        if self.settings.debug:
            alpha = 255.0 / np.percentile(img, 99.5)
            self.save_debug_img(alpha * img.astype(np.float64), 'T_Cell_original.png', False)
            self.save_debug_img(alpha * pref.astype(np.float64), 'T_Cell_prefiltered.png', False)
            
        if self.settings.debug:
            self.save_debug_img(seg3, 'T_Cell_before_postfilter.png', True)

        if self.settings.debug:
            self.save_debug_img(segmentation, 'T_Cell_thresh_final.png', True)
        return segmentation

    def B_cell_segmentation(self, img=None):
        if img is None:
            img = self.get_Bcell_image()
        pref = self.prefilter(img, method=1)
        seg1 = pref > self.settings.thresh_alpha * pref.mean()
        seg2 = remove_small_holes(seg1, 10000)
        seg3 = remove_small_objects(seg2, 40000)
        ...
        if self.settings.debug:
            alpha = 255.0 / np.percentile(img, 99.5)
            self.save_debug_img(alpha * img.astype(np.float64), 'B_Cell_original.png', False)
            self.save_debug_img(alpha * pref.astype(np.float64), 'B_Cell_prefiltered.png', False)
            
        if self.settings.debug:
            self.save_debug_img(seg3, 'B_Cell_before_postfilter.png', True)

        if self.settings.debug:
            self.save_debug_img(segmentation, 'B_Cell_thresh_final.png', True)

        return segmentation

    def display_ilastik_result(self):
        ov = Overlays()
        segmentation = skimage.io.imread(self.settings.ilastik_filename)
        segmentation[segmentation==0] = 1
        segmentation[segmentation==128] = 2
        ...
        skimage.io.imsave(os.path.join(self.settings.output_folder, 
                                       'segmentation_overlay_ilastik_%s.png' % self.settings.dataset),
                          overlay_image)
        return

    def test_loop(self, img):
        for method in [1, 2, 3, 4, 5]:
            print('method ', method)
            start_time = time.time()
            pref = self.prefilter(img, method=method)
            segmentation = pref > pref.mean()
            diff_time = time.time() - start_time
            print('\t time elapsed (in ms): %i' % int(diff_time * 1000))
            if self.settings.debug:
                self.save_debug_img(segmentation, 'pxseg_thresh_method_%i.png' % method, True)
